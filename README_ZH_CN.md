...
2) 无法很好地支持反射。由于它是使用静态分析来实现的，因此它可能会剪裁掉运行时才会被通过反射加载的程序集。
3) 不支持Windows Forms和WPF，而对于程序剪裁功能需求最强烈的其实反而是桌面程序的开发者。
...
2) 在命令行中运行Zack.DotNetTrimmer，把待裁剪的程序的全路径做为参数传递给Zack.DotNetTrimmer。比如：
